...
from mahjong2040.packets import (RonClientPacket,
                                       RonServerPacket,
                                       GameStateServerPacket, Packet)
from mahjong2040.shared import (RON_HONBA_POINTS, ClientGameState,
                                      GameState)
...
class GameRonServerState(BaseGameServerStateMixin):
  ...
  def on_player_ron_complete(self):
    ...
    if self.game_state.player_for_wind(0) in winners:
      self.repeat_hand()
    else:
      self.next_hand()
